...
def read_electron_beam_swarm_from_single_pickle(
    ...
    with open(file_path, mode="rb") as f:
        data = pickle.load(f)
    return __parse_electronbeamswarm_pickle(data, acceleration_data_type, **kwargs)
    ...
    with open(file_path, "rb") as f:
        electron_beam_swarm = electron_beams.ElectronBeamSwarm(
            *__parse_custom_electron_beam_binary_file(
                f, acceleration_data_type, memmap=memmap
            ),
            **kwargs
        )
...
def __parse_custom_field_line_binary_file_memmap(f):
    ...
    if number_of_varying_scalar_quantities > 0:
        ...
        varying_scalar_values = running_memmap(
            f, float_dtype, varying_scalar_values_shape
        )
    ...
    if number_of_varying_vector_quantities > 0:
        ...
        varying_vector_values = running_memmap(
            f, float_dtype, varying_vector_values_shape
        )
...
def __parse_custom_field_line_binary_file_load_all(f):
    ...
    if number_of_field_line_elements > 0:
        split_indices_of_field_line_elements = np.fromfile(
            f, dtype=np.dtype("<u8"), count=number_of_field_lines, sep=""
        )[1:]
    ...
    if number_of_fixed_scalar_quantities > 0:
        ...
        fixed_scalar_values = np.fromfile(
            ...
            sep="",
        ).reshape(fixed_scalar_values_shape)
    ...
    if number_of_fixed_vector_quantities > 0:
        ...
        fixed_vector_values = np.fromfile(
            ...
            sep="",
        ).reshape(fixed_vector_values_shape)
    ...
    if number_of_varying_scalar_quantities > 0:
        ...
        varying_scalar_values = np.fromfile(
            ...
            sep="",
        ).reshape(varying_scalar_values_shape)
    ...
    if number_of_varying_vector_quantities > 0:
        ...
        varying_vector_values = np.fromfile(
            ...
            sep="",
        ).reshape(varying_vector_values_shape)
...
def __parse_fieldlineset3_pickle(data, **kwargs):
    ...
    fixed_scalar_values = __parse_map_of_vec_of_float(data["fixed_scalar_values"])
    fixed_vector_values = __parse_map_of_vec_of_vec3(data.pop("fixed_vector_values"))
    ...
    return field_lines.FieldLineSet3(
        domain_bounds,
        number_of_field_lines,
        fixed_scalar_values,
        fixed_vector_values,
...
def __parse_electronbeamswarm_acceleration_data_pickle(
    acceleration_data, acceleration_data_type
):
    if acceleration_data_type is None:
        acceleration_data = {}
    elif acceleration_data_type == "acceleration_sites":
        acceleration_data = {
            acceleration_data_type: __parse_fieldlineset3_pickle(acceleration_data)
        }
    else:
...
def __parse_cork_pickle(data, **kwargs):
    ...
    scalar_field_values = __parse_vec_of_vec_of_float(data.pop("scalar_field_values"))
    vector_field_values = __parse_vec_of_vec_of_vec3(data.pop("vector_field_values"))
...
def __parse_coords2(data):
    return fields.Coords2(__parse_vec_of_float(data[0]), __parse_vec_of_float(data[1]))
...
def __parse_map_of_vec_of_vec_of_float(data):
    return {name: __parse_vec_of_vec_of_float(vec) for name, vec in data.items()}


def __parse_map_of_vec_of_vec_of_vec3(data):
    return {name: __parse_vec_of_vec_of_vec3(vec) for name, vec in data.items()}
