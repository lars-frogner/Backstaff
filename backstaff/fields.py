...
class ScalarField3:
    ...
    def from_bifrost_data(
        ...
        values = value_processor(
            *(
                get_quantity(quantity)[:, :, ::-1][:, :, k_slice]
    ...
    def at_horizontal_indices_in_bifrost_data(
        ...
        values = value_processor(
            *(
                bifrost_data.get_var(quantity)[i_slice, j_slice, ::-1][:, :, k_slice]
                for quantity in quantities
            )
        )
    ...
    def get_coords(self, inverted_vertically=False):
        return (
            self.get_horizontal_width_coords(),
    ...
    def resampled_along_axis(
        self, axis, resampling_factor, index_range=None, kind="linear"
    ):
        ...
        new_coords[axis] = np.linspace(
            new_coords[axis][0], new_coords[axis][-1], new_axis_size
        )
    ...
    def resampled_to_coords_along_axis(self, axis, new_axis_coords, kind="linear"):
...
class ScalarField1:
    ...
    def resampled_to_coords(self, coords, kind="cubic"):
        values = scipy.interpolate.interp1d(
            self.get_coords(),
            self.get_values(),
            kind=kind,
            copy=False,
    ...
    def get_bounds(self, inverted=False):
        return (
            (-self.coords.coords[-1], -self.coords.coords[0])
            if inverted
            else (self.coords.coords[0], self.coords.coords[-1])
        )
    ...
    def compute_integral(self):
        ...
        return np.sum(0.5 * (values[:-1] + values[1:]) * np.diff(self.get_coords()))
    ...
    def save(self, file_path, compressed=True, overwrite=True):
        if overwrite or not pathlib.Path(file_path).exists():
            if compressed:
                np.savez_compressed(
                    file_path, coords=self.coords.coords, values=self.values
                )
            else:
...
class ScalarField2:
    ...
    def slice_coord_to_idx(bifrost_data, slice_axis, slice_coord):
        all_center_coords = [bifrost_data.x, bifrost_data.y, -bifrost_data.z[::-1]]
        slice_idx = np.argmin(np.abs(all_center_coords[slice_axis] - slice_coord))
        return slice_idx

    @staticmethod
    def slice_from_bifrost_data(
        bifrost_data,
        quantities,
        slice_axis=1,
        slice_coord=7.5,
        ...
        values = value_processor(
            *(
                bifrost_data.get_var(quantity)[:, :, ::-1][
                    all_slices[0], all_slices[1], all_slices[2]
                ]
                for quantity in quantities
            )
        )
        ...
        if isinstance(values, dict):
            ...
            return ScalarFieldSet(
                **{name: ScalarField2(coords, v) for name, v in values.items()}
            )
        else:
            if scale is not None and scale != 1.0:
                values = values * scale
    ...
    @staticmethod
    def accumulated_from_bifrost_data(
        bifrost_data,
        quantities,
        accum_axis=1,
        scale=None,
        ...
        all_values = value_processor(
            *(bifrost_data.get_var(quantity)[:, :, ::-1] for quantity in quantities)
        )
    ...
    def from_file(file_path, field_name_in_set=None):
        try:
            if field_name_in_set is not None:
                return ScalarFieldSet.field_from_file(
                    ScalarField2, file_path, field_name_in_set
                )
    ...
    def get_vertical_bounds(self, inverted=False):
        return (
            (-self.coords.y[-1], -self.coords.y[0])
            if inverted
            else (self.coords.y[0], self.coords.y[-1])
        )
    ...
    def compute_integral(self):
        ...
        return np.sum(
            0.5 * (partial[:-1] + partial[1:]) * np.diff(self.get_vertical_coords())
        )
    ...
    def save(self, file_path, compressed=True, overwrite=True):
        if overwrite or not pathlib.Path(file_path).exists():
            if compressed:
                np.savez_compressed(
                    file_path, x=self.coords.x, y=self.coords.y, values=self.values
                )
            else:
                np.savez(
                    file_path, x=self.coords.x, y=self.coords.y, values=self.values
...
class ScalarFieldSet:
    ...
    def field_from_file(field_class, base_file_path, field_name):
        glob_file_path = ScalarFieldSet._get_field_path(
            base_file_path, "*", escaper=glob.escape
        )
        re_file_path = ScalarFieldSet._get_field_path(
            base_file_path, "(.+)", escaper=re.escape
        )
    ...
    def save(self, file_path, **kwargs):
        for name, field in self.fields.items():
            field.save(self.__class__._get_field_path(file_path, name), **kwargs)
    ...
    @staticmethod
    def _get_field_path(base_file_path, name, separator=".", escaper=lambda x: x):
