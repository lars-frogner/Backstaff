...
class Cork:
    ...
    def exists_at(self, time_idx):
        return (
            time_idx >= self.first_time_idx
            and (time_idx - self.first_time_idx) < self.get_number_of_times()
        )

    def get_time_slice(self):
        return slice(
            self.first_time_idx, self.first_time_idx + self.get_number_of_times()
        )
...
class CorkSet:
    ...
    def __init__(
        ...
        if self.verbose:
            print(
                "Scalar quantity names:\n    {}".format(
                    "\n    ".join(self.scalar_quantity_names)
                )
            )
            print(
                "Vector magnitude names:\n    {}".format(
                    "\n    ".join(self.vector_magnitude_names)
                )
            )
            print(
                "Vector quantity names:\n    {}".format(
                    "\n    ".join(self.vector_quantity_names)
                )
            )
    ...
    def get_scalar_quantity_idx(self, quantity_name):
        return (
            self.vector_magnitude_names.index(quantity_name)
            if quantity_name in self.vector_magnitude_names
            else self.scalar_quantity_names.index(quantity_name)
        )
