...
impl PolyFitInterpolator3 {
        ...
        G: Grid3<F>,
    {
        ...
        let start_offset_x = Self::compute_start_offset(
            &center_coords[X],
            locations[X],
            interp_point[X],
            interp_idx[X],
        );
        let start_offset_y = Self::compute_start_offset(
            &center_coords[Y],
            locations[Y],
            interp_point[Y],
            interp_idx[Y],
        );
        let start_offset_z = Self::compute_start_offset(
            &center_coords[Z],
            locations[Z],
            interp_point[Z],
            interp_idx[Z],
        ...
        let mut start_idx = Idx3::new(
            (interp_idx[X] as isize) + start_offset_x,
    ...
    fn compute_start_offset<F: BFloat>(
        center_coords: &[F],
        ...
        match locations {
            CoordLocation::Center => {
                if Self::ORDER % 2 != 0 && interp_coord < center_coords[interp_idx] {
    ...
    fn create_coordinate_subarray_for_interior<F: BFloat>(
        coords: &[F],
        start_idx: isize,
    ) -> [F; Self::POINTS] {
    ...
    fn create_coordinate_subarray_for_periodic<F: BFloat>(
        coords: &[F],
        start_idx: isize,
    ) -> [F; Self::POINTS] {
        let len = coords.len();
    ...
    fn create_value_subarray_for_interior<F: BFloat>(
        values: &Array3<F>,
        start_idx: &Idx3<isize>,
    ) -> [F; Self::POINTS * Self::POINTS * Self::POINTS] {
    ...
    fn create_value_subarray_for_periodic<F: BFloat>(
        values: &Array3<F>,
        start_idx: &Idx3<isize>,
    ) -> [F; Self::POINTS * Self::POINTS * Self::POINTS] {
        let grid_shape = values.shape();
        let offsets = In3D::new(
            (start_idx[X] + (grid_shape[0] as isize)) as usize,
    ...
    fn interp_subarrays<F: BFloat>(
        coords: In3D<&[F; Self::POINTS]>,
        values: &[F; Self::POINTS * Self::POINTS * Self::POINTS],
        interp_point: &Point3<F>,
    ) -> F {
        ...
        for k in 0..Self::POINTS {
            for j in 0..Self::POINTS {
                vals_c.copy_from_slice(
                    &values[(k * Self::POINTS + j) * Self::POINTS
                        ..(k * Self::POINTS + j + 1) * Self::POINTS],
                );
...
impl Interpolator3 for PolyFitInterpolator3 {
        ...
        G: Grid3<F>,
    {
        ...
        match grid.find_grid_cell(interp_point) {
            GridPointQuery3::Inside(interp_idx) => GridPointQuery3::Inside(Self::interp(
                grid,
                &field.coords(),
                field.locations(),
                field.values(),
        ...
        G: Grid3<F>,
    {
        let grid = field.grid();
        match grid.find_grid_cell(interp_point) {
            GridPointQuery3::Inside(interp_idx) => GridPointQuery3::Inside(Vec3::new(
                Self::interp(
                    grid,
                    &field.coords(X),
                    field.locations(X),
